...
![](images/example_document_revealjs_01.png){.fragment .fade-in-then-out width="60%"}

![](images/example_document_revealjs_02.png){.fragment .fade-in-then-out width="60%"}

![](images/example_document_revealjs_03.png){.fragment .fade-in-then-out width="60%"}
...
````
1. Algunas opciones específicas del formato: 
2. `margin` fija el factor del tamaño de visualización que debe permanecer vacío alrededor del contenido (por defecto 0.1).
[Otros parámetros](https://quarto.org/docs/presentations/revealjs/advanced.html) que controlan el tamaño y margen de las láminas son: `width`, `height`, `min-scale`, `max-scale`. 
3. `slide-number` permite incluir o no los números de las láminas y `scrollable` permite hacer *scrolling* para mostrar contenido que queda fuera de los márgenes normales de una lámina. 
4. `logo` y `footer` permiten incluir una imagen y un pie de página al pie de cada lámina.
5. `css` permite incluir un archivo con estilos **CSS** personalizados, que pueden mejorar la visualización.
...
## Documentos Quarto | Ejercicio

::: {.small-par}
1) **TODO:** add list of short exercises to edit and render some of the presented docs.
:::
...
- Comportamiento del bloque de código puede ser configurado usando comentarios especiales al comienzo del bloque (por ejemplo: echo, eval).
  - **TODO:** Diferencia con R Markdown.
:::
...
## Dashboards | Ejercicio

::: {.small-par}
1)  Trabajaremos en una copia del mismo dashboard que hemos visto en las láminas anteriores y usaremos el dataset **gapminder**.
    - Cree una copia del archivo **dashboard_example.qmd** y llámela **dashboard_example_ejercicio.qmd**.
...
4)  En la segunda columna (derecha), cree dos tabsets.
    - El primer tabset debería mostrar un gráfico de la expectativa de vida en el tiempo, para los tres países.
